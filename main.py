...
async def on_ready():
    ...
    await app.change_presence(status=discord.Status.online, activity=game) # 온라인 상태, game 중으로 설정
...
async def 중식(ctx):
    ...
    embed = discord.Embed(title="**Whats Meal**", description='', color=15105570)
...
async def 도움(ctx):
    embed = discord.Embed(title="**KS Meal Notification**", description="경상고 급식 알리미 봇 입니다\n", color=0x62c1cc)
    embed.add_field(name="**!중식**", value="`오늘의 점심메뉴를 안내합니다.`", inline=True)
    embed.add_field(name="**!석식**", value="`오늘의 저녁메뉴를 안내합니다.`", inline=True)
    embed.add_field(name="**!메뉴 <mm> <dd>**", value="`지정한 날짜의 급식을 안내합니다.\n 현재 석식은 제공하지 않습니다.`", inline=False)
    embed.add_field(name="**!예시**", value="`명령어 사용 예시를 보여줍니다`", inline=False)
    embed.set_footer(text="아키티오#9732 [도움 : khw#2662]",icon_url="https://i.imgur.com/gqhyveT.png")
    await ctx.send(embed=embed)
...
async def 석식(ctx):
    ...
    if c == "None":
        c = "오늘의 석식이 없습니다"
    embed = discord.Embed(title="**Whats Meal**", description='', color=15105570)
    embed.add_field(name=getMeal.dateInfo()+"의 저녁메뉴",value="```"+c+"```",inline=False)
    await ctx.send(embed=embed)

@app.command(description='For when you wanna know meal menu with specific date')
async def 메뉴(ctx, *args):
    if not args:
        ...
        embed = discord.Embed(title="**Whats Meal**", description='', color=15105570)
        embed.add_field(name=args[0]+"/"+args[1]+"의 메뉴(중식)",value=c,inline=False)
        await ctx.send(embed=embed)

@app.command(description='For when you wanna know how to use commands')
async def 예시(ctx):
    embed = discord.Embed(title="**Whats Commands**", description='', color=2067276)
